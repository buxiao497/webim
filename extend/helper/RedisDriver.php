...
class RedisDriver {
	...
	protected $ip = '119.29.208.229'; // redis服务器ip地址
	protected $port = '6379'; // redis服务器端口
	protected $passwd = '123456'; // redis密码
	...
	// 推送一条数据至列表，头部
	public function pushList($key, $arr, $expire = null) {
		$key = 'list:' . $key;
	...
	// 推送一条数据至列表，尾部
	public function rpushList($key, $arr, $expire = null) {
		$key = 'list:' . $key;
		$this->redis->rPush($key, json_encode($arr));
		if (!is_null($expire)) {
			$this->redis->setTimeout($key, $expire);
		}
	}
	// 从列表拉取一条数据，头部
	public function lpullList($key, $timeout = 0) {
		$key = 'list:' . $key;
		if ($timeout > 0) {
			$val = $this->redis->blPop($key, $timeout); // 该函数返回的是一个数组, 0=key 1=value
		} else {
	...
	// 从列表拉取范围数据库
	public function lrangeList($key, $limit, $page) {
		$key = 'list:' . $key;
		$val = $this->redis->lRange($key, $limit, $page);
		// return $val;
		// var_dump($val);exit;
		$row = array();
		if (is_array($val)) {
			foreach ($val as $key => $value) {
				$row[] = $this->objectToArray(json_decode($value));
			}
		}
		return $row;
		// $val = is_array($val) && isset($val[1]) ? $val[1] : $val;
